...
| 是否已进入 `main` | 否 |
| 当前定位 | 可供后续开发的 V1 集成候选基线，不是正式生产发布版 |
...
但该页面仍保留 SEED 演示数据和 localStorage 降级逻辑，因此它是可联调的本地 V1 UI 基线，不应被描述为完整生产前端工程。

部署基线包括：

- `Dockerfile`：可构建 FastAPI 应用镜像；
- `compose.yaml`：可启动 PostgreSQL 16 + pgvector；
- Alembic 迁移；
- 演示数据初始化 CLI。
...
### 12.1 真实飞书 OAuth 权限链路未完整

当前 Live OAuth 可以交换用户 Token 和读取用户信息，但授权 URL 没有声明业务 scope 和 `offline_access`。因此真实登录可能成功，文档、妙记读取和 refresh token 能力仍可能因权限不足失败。

### 12.2 飞书应用身份 Token 未实现

当前 `LiveFeishuAdapter` 主要持有用户 Token。机器人创建群需要应用身份 `tenant_access_token`，当前没有独立的 tenant token 获取、缓存和刷新 Provider。

### 12.3 飞书原生事件入口未实现

当前 `/expert-collaborations/events/replies` 接收项目内部 `ExpertReplyEvent`，不是飞书原生事件结构。尚未实现：

- URL Verification `challenge`；
- 飞书 v2 事件头解析；
- `im.message.receive_v1` 消息解析；
- Encrypt Key 解密或签名校验；
- 群、协作、问题和回复之间的原生消息映射。

现有内部回复接口不能直接填入飞书开放平台作为生产回调地址。

### 12.4 飞书 Token 未加密存储

数据库字段已经存在，但当前 Token 以明文形式保存。正式云部署前需要增加应用层加密、密钥轮换和日志脱敏。
...
当前 Compose 只有 PostgreSQL 服务，尚未包括：
...
当前 UI 是单文件静态页面，包含演示数据和降级状态。尚未完成独立构建、前端路由、组件化、前端测试体系、生产错误监控和完整真实数据验收。
...
|---|---|---|---|---|
| 1 | `feat/feishu-oauth-scope` | 补齐 scope、offline_access 和权限错误处理 | 现有 Auth/Adapter | 真实测试企业登录、刷新和文档读取通过 |
| 2 | `feat/feishu-tenant-token` | 增加 tenant token Provider | App ID/Secret | 真实机器人创建测试群并发送消息 |
| 3 | `feat/feishu-event-callback` | 实现 challenge、验签和消息事件归一化 | 飞书测试应用 | 群内回复进入内部 ExpertReplyEvent |
| 4 | `feat/token-encryption` | Token 加密、轮换和脱敏 | 数据库迁移 | 数据库不再保存可直接使用的明文 Token |
| 5 | `feat/research-worker` | 持久化任务调度和重启恢复 | 现有 Pipeline | 重启服务后未完成任务可继续 |
| 6 | `feat/cloud-deployment` | app、Postgres、HTTPS、迁移和备份 | 云服务器/域名 | 公网 HTTPS 健康检查和迁移通过 |
| 7 | `feat/frontend-production` | 清理演示降级并完成真实接口状态 | 稳定 API | 核心页面只使用真实数据通过验收 |
| 8 | `test/feishu-e2e` | 真实飞书完整链路验收 | 上述全部 | 专家回复恢复研究并生成最终报告 |

可以并行的工作：OAuth 与云部署基础设施可以由不同开发者并行；事件回调依赖飞书测试应用，生产前端依赖稳定 API。共享契约、数据库迁移和 `app/api/deps.py` 冲突必须共同评审。
...
当前 V0.5、AI 服务、V1 Deep Research 和专家协作后端已经形成基线；
真实飞书权限、tenant token、原生事件回调、Token 加密、持久化 Worker
和生产云部署尚未完成。
