...
### 12.1 真实飞书 OAuth 权限链路已补齐代码，待真实应用验收

Live OAuth 已使用新版`client_id`、`response_type=code`、业务 scope 和`offline_access`，并支持用户 Token 刷新。文档元数据/正文、妙记搜索/信息/文字转写已按官方接口拆分。仍需使用真实飞书应用凭证、已发布权限和测试成员完成端到端验收。
