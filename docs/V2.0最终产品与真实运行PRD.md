...
| Open Enrich Sidecar（可选） | URL、内部 Token、采集/模型凭据 | 组合搜索禁用；阿里云默认搜索和已入库情报不受影响 |
| 展示解析 Provider | `APP_PRESENTATION_SERVICE_URL` | 历史参考稿解析跳过；不影响已有可信 HTML |

## 6. 内置文档索引验收

内置文档位于 `fixtures/builtin_documents/index_cases.json`，包含两条经验和两条能力，并固定写入与真实业务不同的隔离 workspace。运行：
...
## 公开情报自动搜索增量（2026-08-15）

情报工作台在保留手工真实来源入口的基础上，新增阿里云百炼 Web Search 自动发现入口。百炼只负责发现公开 URL，系统必须继续完成公网安全校验、正文抓取、正文哈希、RawArtifact 快照、待审核情报项、情报快照和人工画像审批。

自动搜索不可用时必须明确失败，不得回退到 Mock 搜索结果。搜索摘要不能直接作为客户事实、企业能力或历史经验；只有正文成功抓取并完成来源绑定的内容才能进入后续抽取。接口、配置、失败语义与迁移要求见 `docs/V2百炼公开情报搜索接入说明.md`。

比赛环境以阿里云搜索为默认主链路。Open Enrich 仅作为可选补充 Provider，用于客户画像公开来源的覆盖率对照和交叉验证；其结果仍须由后端重新抓取引用 URL、绑定 RawArtifact、校验 quote、执行去重冲突治理并生成待人工确认的 Proposal。Open Enrich 未配置或执行失败不得阻断、清除或降级已完成的阿里主结果。
