...
def _asset_lookup(snapshot: RetrievalSnapshot) -> dict[tuple[str, str], Any]:
    return {
...
def build_evidence_candidates(
    ...
    for asset in _asset_lookup(snapshot).values():
...
def _compatible_evidence_ids(
    ...
    if boundary not in {
        ...
        return set()
    expected_prefix = "EXP-" if boundary == EvidenceBoundary.HISTORICAL_FACT else "CAP-"
    return {item.evidence_id for item in candidates if item.asset_id.startswith(expected_prefix)}
